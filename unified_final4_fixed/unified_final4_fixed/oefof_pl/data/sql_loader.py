...
def load_nav_history_sql(
    ...
    *,
    ...
    """Load daily NAV history for one fund code over ``start < date <= end``."""
    if len(start_yyyymmdd) != 8 or not start_yyyymmdd.isdigit():
        raise ValueError(f"start_yyyymmdd must be 8 digits, got {start_yyyymmdd!r}")
    if len(end_yyyymmdd) != 8 or not end_yyyymmdd.isdigit():
        raise ValueError(f"end_yyyymmdd must be 8 digits, got {end_yyyymmdd!r}")
    for label, ident in (
        ...
        ("vdate_column", vdate_column),
        ("nav_value_column", nav_value_column),
    ):
        _assert_safe_sql_identifier(ident, label)

    try:
        import pyodbc  # type: ignore
    except ImportError as e:  # pragma: no cover
        raise DataLoadError(
            "pyodbc is required for NAV history loading. "
            "Install with: pip install pyodbc"
        ) from e
    ...
    end_iso = f"{end_yyyymmdd[:4]}-{end_yyyymmdd[4:6]}-{end_yyyymmdd[6:8]}"
    sql = (
        f"SELECT {vdate_column} AS VDATE, SUM({nav_value_column}) AS NAV_EUR "
        f"FROM {nav_table} "
        f"WHERE UPPER({pcode_column}) LIKE ? "
        f"AND CAST({vdate_column} AS date) > CAST(? AS date) "
        f"AND CAST({vdate_column} AS date) <= CAST(? AS date) "
        f"GROUP BY {vdate_column}"
    )
    params = (str(pcode_pattern).strip().upper(), start_iso, end_iso)
    where = f"{nav_table} {start_iso}<{end_iso} pcode_like={pcode_pattern}"
    ...
    df = pd.DataFrame.from_records([tuple(r) for r in rows], columns=["VDATE", "NAV_EUR"])
    if df.empty:
        raise EmptyDataError(f"{where}: no NAV rows returned.")

    df["VDATE"] = pd.to_datetime(df["VDATE"], errors="coerce")
    df["NAV_EUR"] = pd.to_numeric(df["NAV_EUR"], errors="coerce")
    df = df.dropna(subset=["VDATE", "NAV_EUR"]).sort_values("VDATE").reset_index(drop=True)
    if df.empty:
        raise EmptyDataError(f"{where}: NAV rows were present but unusable after parsing.")
    return df
